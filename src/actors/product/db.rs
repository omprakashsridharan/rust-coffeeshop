...
mod test {
    ...
    use super::{get_all, get_by_names};
    ...
    #[sqlx::test(fixtures("get_products_by_names"))]
    async fn get_by_names_test_unavailable_types(pool: PgPool) {
        let products_result = get_by_names(pool, vec![String::from("TABLE")]).await;
        assert!(products_result.is_ok());
        let products = products_result.unwrap();
        assert_eq!(0, products.len());
    }
}